...
(function () {
      ...
      btn.addEventListener('click', async (e) => {
        ...
        try {
          ...
          sessionStorage.setItem('last_order', JSON.stringify({
            ...
            invoice_file: r.invoice_file || '',            invoice_url: r.invoice_url || ('/api/orders/' + r.order_id + '/invoice'),
          }));
  ...
  const wiring = {
    ...
    'cart.html': async function () {
      ...
      function render(items) {
        if (!items.length) {
          ...
          if (totalWrap) totalWrap.style.display = 'none';
        } else {          if (totalWrap) totalWrap.style.display = '';
    ...
    'payment-success.html': function () {
      ...
      try {
        ...
        const receipt = $('#invoiceReceipt');        if (receipt) receipt.classList.remove('d-none');
  ...
  async function wireAllProductsPage() {
    const grid = $('#allProductsGrid');    const categoryWrap = $('#allProductsCategories');
  ...
  async function wireProductReviews() {
    ...
    const renderReviews = async () => {
      ...
      list.innerHTML = reviews.map((review) => (
        ...
        escapeHtml(review.username) + ' · ' + escapeHtml(review.created_at) + '</span></div></li>'      )).join('') || '<li class="single-user-review">No verified reviews yet.</li>';
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    wireProductDetail().catch(() => {});    wireHomepageSections().catch(() => {});
    await loadSession();
